...
# 2025년 10월 13일 — TCP 성능 고려 및 HTTP 커넥션 관리 도입

⸻

## 4.2 TCP 성능에 대한 고려

HTTP는 **TCP 위에서 동작하는 프로토콜**이므로,  
HTTP 트랜잭션의 성능은 TCP의 성능에 직접적인 영향을 받는다.

따라서 TCP의 기본적인 특성과 성능 요인을 이해하면,  
HTTP 커넥션 최적화(keep-alive, 파이프라이닝, 멀티플렉싱 등)에 유리하다.

> TCP 성능에 대한 세부 기술적 부분에 관심이 없다면,  
> “HTTP 커넥션 관리”로 바로 넘어가도 무방하다.

---

## 4.2.1 HTTP 트랜잭션 지연

HTTP 요청/응답 과정에서 발생하는 **네트워크 지연(latency)** 은  
대부분 TCP 레벨에서 발생한다.

HTTP 트랜잭션의 흐름은 다음과 같다:

| 구분                           | 과정                                                                                       |
| ------------------------------ | ------------------------------------------------------------------------------------------ |
| **DNS 조회**                   | 클라이언트가 URI에서 서버의 IP 주소를 얻기 위해 DNS 요청을 수행한다. (수십 ms ~ 수초 소요) |
| **TCP 연결 (3-way handshake)** | 클라이언트가 서버에 커넥션 요청을 보내고 응답을 기다린다. (1~2초 소요 가능)                |
| **요청 전송**                  | 클라이언트가 HTTP 요청 메시지를 TCP를 통해 서버로 전송한다.                                |
| **서버 처리**                  | 서버는 요청을 처리하고 응답 메시지를 생성한다.                                             |
| **응답 전송**                  | 서버가 HTTP 응답을 클라이언트로 보낸다.                                                    |
| **커넥션 종료**                | 클라이언트가 응답을 모두 받은 후 TCP 연결을 종료한다.                                      |
...
1. **DNS 지연**
   ...
   - 호스트 이름을 IP로 변환해야 함.
   - 캐시가 없거나 네트워크가 혼잡한 경우 수십 초까지 지연 가능.

2. **TCP 커넥션 설정 지연**

   - 새로운 TCP 커넥션마다 **3-way handshake** 과정 필요.
   - 각 트랜잭션마다 새 연결을 맺으면 지연 누적.

3. **데이터 전송 지연**

   - 요청과 응답 메시지가 네트워크를 통해 이동하는 데 걸리는 시간.
   - 서버의 처리 속도, 네트워크 대역폭, 거리(latency)에 영향받음.

4. **응답 처리 지연**
   - 서버가 응답을 생성하고 클라이언트가 이를 수신하는 과정.

> 대부분의 HTTP 지연은 **TCP 네트워크 지연**에서 발생하며,  
> 클라이언트/서버 간 거리, 네트워크 혼잡, 메시지 크기 등에 의해 달라진다.

---

## 4.2.2 성능 관련 주요 요소

다음은 **HTTP 프로그래머가 성능 최적화 시 반드시 고려해야 할 TCP 요인들**이다.

| 구분                                       | 설명                                                                                  |
| ------------------------------------------ | ------------------------------------------------------------------------------------- |
| **1. TCP 핸드셰이크 설정**                 | 커넥션 생성 시 3-way handshake로 인해 초기 지연 발생.                                 |
| **2. 느린 시작(Slow Start)**               | 네트워크 혼잡 방지를 위해 전송 속도를 점진적으로 증가시키는 알고리즘.                 |
| **3. 네이글 알고리즘 (Nagle’s Algorithm)** | 작은 패킷을 모아 한 번에 전송하여 효율 향상, 그러나 실시간 전송에서는 지연 유발 가능. |
| **4. 확인응답 지연 알고리즘 (ACK Delay)**  | 수신자가 여러 패킷에 대한 응답을 한꺼번에 보내기 위한 알고리즘.                       |
| **5. TIME_WAIT 지연 & 포트 고갈**          | 커넥션 종료 후 일정 시간 동안 포트를 재사용할 수 없어 새로운 연결에 영향을 줌.        |

> 고성능 HTTP 서버(예: Nginx, Node.js, Apache 등)를 개발하거나 운영한다면  
> 위의 다섯 가지 TCP 요인을 반드시 이해하고 있어야 한다.

---

📌 **참고 요약**

- 대부분의 HTTP 성능 문제는 **네트워크 레벨의 TCP 병목**에서 비롯된다.
- **HTTP/2, HTTP/3** 등의 기술은 이러한 TCP 한계를 극복하기 위해 등장했다.
- 각 TCP 지연 요소는 **전송 효율 vs 실시간성** 간의 트레이드오프를 가진다.

---

## 4.3 HTTP 커넥션 관리 (도입)

앞선 절에서는 TCP 커넥션의 구조와 성능적 영향을 살펴보았다.  
이제부터는 **HTTP 계층에서의 커넥션 관리와 최적화 기법**을 다룬다.

핵심 학습 목표:

- HTTP 커넥션의 생성과 유지 과정 이해
- Keep-Alive와 파이프라이닝의 작동 원리
- 커넥션 헤더(`Connection`, `Keep-Alive`, `Close`)의 정확한 동작 방식
- 커넥션 병렬화 및 재사용 최적화 기법

> **다음 절(4.3.1)**에서는 흔히 오해되는 `Connection` 헤더의 실제 의미부터 다룰 예정.

⸻

# 📖 2025년 10월 14일 — Connection 헤더와 커넥션 성능 향상 기술

⸻

## 4.3.1 흔히 잘못 이해하는 Connection 헤더

HTTP는 클라이언트와 서버 사이에 **프록시, 캐시 등 중개 서버**가 존재할 수 있다.  
이때 `Connection` 헤더는 단순히 “연결 유지 여부”가 아니라,  
**“현재 커넥션에만 유효한 정보”** 를 정의한다.

### Connection 헤더에 들어갈 수 있는 3가지 토큰

| 종류             | 의미                          | 설명                               |
| ---------------- | ----------------------------- | ---------------------------------- |
| **헤더 필드 명** | 현재 커넥션에만 적용되는 헤더 | 다음 홉(서버·프록시)으로 전달 금지 |
| **임시 토큰 값** | 비표준 또는 커넥션 특화 옵션  | 예: 실험적 TCP 확장                |
| **`close` 값**   | 현재 요청/응답 후 연결 종료   | `Connection: close`                |

### 핵심 개념 — 헤더 보호(Header Protection)

- `Connection` 헤더에 명시된 모든 헤더 필드는 **홉별(Hop-by-Hop)** 헤더이다.
- 즉, 다음 서버로 전달될 때 **삭제되어야 한다.**
- 이는 프록시가 잘못된 상태 정보를 계속 전달하는 걸 방지하기 위함이다.

> 정리: `Connection` 헤더는 **현재 커넥션 한정 정보**를 정의하며, 다음 홉으로 전달되지 않는다.

### 개발자 인사이트

- HTTP/1.1에서는 `Connection: keep-alive`가 **기본 동작**이다.
- HTTP/2 이상에서는 **멀티플렉싱**으로 이 개념이 사라진다.
- DevTools 네트워크 탭에서 `Connection: keep-alive`가 보인다면,  
  이는 “TCP 세션 재활용 가능” 상태임을 의미한다.
...
HTTP/1.0은 **요청마다 새 TCP 커넥션**을 맺었다.  
→ 각 요청마다 핸드셰이크, 느린 시작, 종료 대기(TIME_WAIT) 등의 지연이 발생.

예: HTML 1개 + 이미지 3개 = 4번의 커넥션  
→ 모든 요청을 순차 처리 → 전송 지연 + 빈 화면 + 심리적 지연

### 지연 유형

| 유형              | 설명                                                    |
| ----------------- | ------------------------------------------------------- |
| **물리적 지연**   | 커넥션 생성·종료 반복으로 전송 시간 증가                |
| **심리적 지연**   | 이미지가 순차적으로 뜨면 느리다고 느낌                  |
| **브라우저 지연** | 객체 크기 확인 전 배치 불가 → 전체 로딩 전 빈 화면 노출 |
...
| 기술명                                     | 개념                                     | 장점                             |
| ------------------------------------------ | ---------------------------------------- | -------------------------------- |
| **① 병렬 커넥션 (Parallel Connections)**   | 여러 TCP 커넥션으로 동시 요청 수행       | 병렬 다운로드로 초기 렌더링 향상 |
| **② 지속 커넥션 (Persistent Connections)** | 커넥션을 재사용하여 지연 제거            | Handshake/Slow-Start 제거        |
| **③ 파이프라인 커넥션 (Pipelining)**       | 하나의 커넥션에 여러 요청을 순차 전송    | RTT 감소, 단 HOL Blocking 문제   |
| **④ 다중 커넥션 (Multiplexing)**           | 하나의 커넥션 내에서 여러 요청/응답 병행 | HTTP/2에서 완성, 최상 효율       |

---

## 정리 & 인사이트

> 커넥션 관리가 HTTP 성능 최적화의 핵심이다.  
> `Connection` 헤더는 “어떤 정보가 다음 홉으로 전달되지 말아야 하는가”를 규정하고,  
> 병렬·지속·파이프라인·다중 커넥션은 “전송 효율을 어떻게 높일 것인가”를 해결한다.

좋아요. 아래는 오늘 정민님이 공부한 HTTP 4.4~4.5 (병렬 커넥션 & 지속 커넥션) 내용을
그대로 Markdown(.md) 형식으로 정리한 버전이에요.
노션, 깃허브, 혹은 밀리의 서재 메모처럼 바로 붙여넣기 하시면 됩니다.

⸻

# 📖 2025년 10월 15일 — HTTP 커넥션 (병렬 커넥션 & 지속 커넥션)

## 4.4 병렬 커넥션 (Parallel Connections)

### 개요

브라우저는 HTML 문서를 받은 뒤 포함된 이미지, CSS, JS 파일 등을 순차적으로 요청할 수 있지만,
이 방식은 느리다.  
HTTP는 여러 개의 커넥션을 맺어 여러 HTTP 트랜잭션을 병렬로 처리할 수 있다.

---

### 4.4.1 병렬 커넥션은 페이지를 더 빠르게 만든다

- 각 객체(이미지, CSS, JS 등)를 **서로 다른 TCP 커넥션**으로 동시에 전송.
- 커넥션의 지연 시간을 **겹치게(overlap)** 하여 총 지연 시간을 단축한다.
- 한 커넥션이 대기 중일 때 다른 커넥션으로 데이터를 받아 대역폭 활용 효율을 높인다.

**핵심 요약**

- 대역폭 활용 극대화
- 지연시간 겹침 효과
- 병렬 처리를 통해 총 다운로드 시간을 줄임

---

### 4.4.2 병렬 커넥션이 항상 빠른 것은 아니다

- **대역폭이 충분하지 않을 경우** 오히려 병렬 커넥션이 느려질 수 있다.
- 저속 네트워크에서는 여러 요청이 동시에 대역폭을 나누어 쓰기 때문.
- 커넥션이 많아질수록 서버의 메모리/CPU 사용량 증가.
- 복잡한 페이지는 수백 개의 객체를 포함할 수 있어, 과도한 커넥션은 서버 부하를 초래.

**예시**

- 100명의 사용자가 각각 100개의 커넥션을 맺는다면  
  → 서버는 총 **10,000개의 연결**을 관리해야 함.

**브라우저 제한**

- 대부분의 브라우저는 **한 서버당 4~6개의 병렬 커넥션**만 허용.
- 서버는 클라이언트의 과도한 연결 시 임의로 커넥션을 끊을 수 있다.

---

### 4.4.3 병렬 커넥션은 "더 빠르게 느껴질 수 있다"

- 실제 다운로드 총 시간은 더 길 수 있다.
- 하지만 **여러 객체가 동시에 로드되는 시각적 효과**로 인해 사용자는 더 빠르게 느낀다.

> 💡 **개인 메모:**  
> 이런 UX 효과 때문에 **스켈레톤 로딩(Skeleton Loading)** 같은 기법이 사용된다.  
> 실제보다 빠르게 보이게 만드는 시각적 피드백 방식이다.  
> (최근에는 스켈레톤이 너무 오래 보이면 오히려 느리게 느껴지기도 함.)

## 4.5 지속 커넥션 (Persistent Connections)

### 개요

- 대부분의 웹페이지는 동일한 서버에서 여러 리소스를 요청한다.
- HTML, 이미지, CSS, JS 등이 모두 같은 도메인일 경우가 많다.
- 이 성질을 **사이트 지역성(Site Locality)**이라고 한다.
- HTTP/1.1부터는 커넥션을 끊지 않고 **재사용** 가능하게 했다.  
  → 이를 **지속 커넥션(Persistent Connection)**이라 한다.

---

### 비지속 커넥션 (Non-persistent Connection)

- HTTP/1.0 방식.
- 요청 1건당 커넥션 1개를 사용.
- 요청마다 TCP 핸드셰이크와 종료 과정이 발생 → 비효율적.
...
- HTTP/1.1에서 기본 동작으로 설정됨.
- 한 번 맺은 TCP 커넥션을 여러 요청/응답에 재사용.
- 클라이언트나 서버가 명시적으로 끊기 전까지는 커넥션을 유지.

**장점**
| 항목 | 설명 |
|------|------|
| 핸드셰이크 생략 | 매 요청마다 새 TCP 커넥션을 생성할 필요 없음 |
| 지연 감소 | 커넥션 재활용으로 전송 속도 향상 |
| TCP 느린 시작 회피 | 이미 열린 커넥션은 빠른 전송 속도 유지 |
| 서버 부하 감소 | 커넥션 생성/종료의 CPU 비용 절약 |
...
- **병렬 커넥션**: 여러 개의 커넥션으로 동시에 요청 (속도 향상 but 리소스 부담)
- **지속 커넥션**: 한 커넥션을 여러 요청에서 재사용 (효율적, 표준 방식)
- HTTP/1.1 이후의 기본 구조는  
  → “지속 커넥션 + 적절한 병렬 처리” 조합
...
- HTTP/1.0 → 요청당 커넥션 (비효율적)
- HTTP/1.1 → Keep-Alive 기본 활성화
- HTTP/2 → 단일 커넥션 내 다중 스트림 (Multiplexing)
- HTTP/3 (QUIC) → TCP 한계를 극복한 지연 최소화 구조
